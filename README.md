...
* Corrected `customer\_unique\_id` (the actual per-shopper identifier) in place of the per-order `customer\_id` field, which the dataset's schema makes easy to use incorrectly, to get a real repeat-purchase rate.
...
Full reasoning, quantified impact, and tracking metrics for each recommendation are in [`reports/Olist\_Delivery\_Performance\_Report.docx`](reports/Olist_Delivery_Performance_Report.docx).
...
```
├── notebooks/
│   └── olist\_eda.ipynb              EDA notebook — cleaning, delay analysis, hypothesis testing
├── reports/
│   └── Olist\_Delivery\_Performance\_Report.docx
├── dashboards/
│   └── screenshots/
└── README.md
```
...
[Olist Brazilian E-Commerce Public Dataset](https://www.kaggle.com/datasets/olistbr/brazilian-ecommerce) (Kaggle). Not included in this repo — download separately to reproduce.
